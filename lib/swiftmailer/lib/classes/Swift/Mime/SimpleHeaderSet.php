...
class Swift_Mime_SimpleHeaderSet implements Swift_Mime_CharsetObserver
{
    ...
    {
        if (!isset($this->headers[strtolower((string)$name)])) {
            $this->headers[strtolower((string)$name)] = [];
        }
        if (!isset($offset)) {
            $this->headers[strtolower((string)$name)][] = $header;
        } else {
            $this->headers[strtolower((string)$name)][$offset] = $header;
        }
    }
    ...
    {
        return array_key_exists(strtolower((string)$header->getFieldName()), $this->required);
    }
